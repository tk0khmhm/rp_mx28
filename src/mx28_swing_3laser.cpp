...
pthread_mutex_t mutex_shared_data = PTHREAD_MUTEX_INITIALIZER;
...
void MX28_write(char ID, int start, int bytes, char* data, int fd)
{
	...
	int status_length = 0;
	//int status_length = 6;
	//int status_length = serialDataAvail(fd);
	status_length = serialDataAvail(fd);
	//while((status_length = serialDataAvail(fd))){
...
double angle21 = 0.0;
double angle22 = 0.0;
void* read_encorder(void* args)
{
	while(1){
		puts("hey");
		//pthread_mutex_lock(&mutex_shared_data);
		//angle20 = ReadPresentPosition(20, fd);
...
int main(int argc, char** argv)
{
	...
	pthread_t thp;
	pthread_create(&thp, NULL, read_encorder, NULL);
	...
	char id21 = char(21);
	pthread_mutex_lock(&mutex_shared_data);
	MX28SetMovingSpeed(id20, rpm, fd);
	MX28SetMovingSpeed(id21, rpm, fd);
	pthread_mutex_unlock(&mutex_shared_data);
	...
	wiringPiISR( T_PIN20, INT_EDGE_FALLING, interrupt);
	wiringPiISR( T_PIN21, INT_EDGE_FALLING, interrupt);
	wiringPiISR( T_PIN22, INT_EDGE_FALLING, interrupt);
	...
	pthread_mutex_lock(&mutex_shared_data);
	MX28SetMovingSpeed(id20, 0, fd);
	MX28SetMovingSpeed(id21, 0, fd);
	pthread_mutex_unlock(&mutex_shared_data);
