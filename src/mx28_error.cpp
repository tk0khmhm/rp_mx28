...
#define EEPROM_FIRMWARE_VERSION				0x02	// 1byte
...
#define EEPROM_ALARM_LED					0x11	// 1byte
...
#define RAM_HIGHEST_LIMIT_TORQUE			0x22	// 1byte
#define RAM_LOWEST_LIMIT_TORQUE				0x23	// 1byte
...
void SetHighTorqueLimit(char id, double ht_angle_limit, int fd)
{
	short data;
	char byte[2];
	data = (short)( ht_angle_limit );
	cout<<"ht_limit:"<<data<<endl;
	byte[0] = 0xff & data;
	byte[1] = data >> 8;
	MX28_write(id, RAM_HIGHEST_LIMIT_TORQUE, 2, byte, fd);
}

void SetLowTorqueLimit(char id, double lt_angle_limit, int fd)
{
	short data;
	char byte[2];
	data = (short)( lt_angle_limit );
	cout<<"lt_limit:"<<data<<endl;
	byte[0] = 0xff & data;
	byte[1] = data >> 8;
	MX28_write(id, EEPROM_CCW_ANGLE_LIMIT, 2, byte, fd);
}
...
float ReadAlarm(char id, int fd)
{
	char TxBuf[16];
	char sum = 0;
	char RxBuf[16];
	...
	//TxBuf[5] = EEPROM_ALARM_LED;
	//TxBuf[5] = EEPROM_HIGHEST_LIMIT_TEMPERATURE;
	TxBuf[5] = EEPROM_FIRMWARE_VERSION;
	sum += TxBuf[5];
...
int main(int argc, char** argv)
{
	...
	char* opt_str = {(char*)"p:i:y:b:z:jwl:m:a:c:r:d:t:eh"};
	struct option long_opts[] = {
		...
		{"h-torque-lim",required_argument,	0,	'a'},
		{"l-torque-lim",required_argument,	0,	'c'},
	...
	while( (c = getopt_long(argc, argv, opt_str, long_opts, &opt_idx)) != -1 ){
		switch (c){
			...
			case'm':
				...
				break;
			case'a':
				ht_limit = (double)atof(optarg);
				ht_f = 1;
				cout<<"ht_limit:"<<ht_limit<<endl;
				break;
			case'c':
				lt_limit = (double)atof(optarg);
				lt_f = 1;
				cout<<"lt_limit:"<<lt_limit<<endl;
				break;
			case 'r':
			...
			case 'h':
				...
				printf("    --ltorquelimit  -a: [double] \n");
				printf("    --htorquelimit  -c: [double] \n");
				printf("    --rpm           -r:\n");
				printf("    --degree        -d:\n");
				printf("    --timing        -t:\n");
				printf("    --error        -t:\n");
	...
	if(ht_f){
		cout<<"Changing H_TORQUE_LIMIT"<<endl;
		SetHighTorqueLimit(id, ht_limit, fd);
		return 0;
	}
	if(lt_f){
		cout<<"Changing L_TORQUE_LIMIT"<<endl;
		SetLowTorqueLimit(id, lt_limit, fd);
		return 0;
	}
	if(gp_f){
